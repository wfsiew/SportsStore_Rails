...
class RouteTest < ActionController::TestCase
  ...
  test "should route to cart" do
    ...
    assert_routing({ :method => :post, :path => '/cart/add/2' }, 
                   { :controller => 'cart', :action => 'add', :id => '2' })
    assert_routing({ :method => :post, :path => '/cart/remove/2' }, 
                   { :controller => 'cart', :action => 'remove', :id => '2' })
  ...
  test "should route to admin" do
    ...
    assert_routing({ :method => :post, :path => '/admin/product/create' }, 
                   { :controller => 'admin/product', :action => 'create' })
    assert_routing '/admin/product/edit/3', { :controller => 'admin/product', :action => 'edit', :id => '3' }
    assert_routing({ :method => :put, :path => '/admin/product/update/3' }, 
                   { :controller => 'admin/product', :action => 'update', :id => '3' })
  ...
  test "should route to mobile" do
    ...
    assert_routing({ :method => :post, :path => '/mobile/product/list/8' }, 
                   { :controller => 'mobile/product', :action => 'list_paged', :page => '8' })
